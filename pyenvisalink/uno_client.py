...
class UnoClient(HoneywellClient):
        ...
        while idx < num_bytes:
            ...
            for bit in range(8):
                ...
                _LOGGER.debug(
                    "(zone %i) is %s",
                    zoneNumber,
                    "Open/Faulted" if faulted else "Closed/Not Faulted",
                )
    ...
    def handle_partition_state_change(self, code, data):
        ...
        for currentIndex in range(0, 8):
            ...
            if not partitionState:
                _LOGGER.warn(
                    "Unrecognized partition state code (%s) received for partition %d",
                    str(partitionStateCode),
                    partitionNumber,
                )
                continue
    ...
    def handle_host_information_report(self, code, data):
        ...
        version = host_info[2]
        _LOGGER.debug(
            "Host info: MAC=%s, Device Type=%s, Version='%s'", mac_address, device_type, version
        )
        return
